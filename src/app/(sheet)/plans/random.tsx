...
export default function RandomPlansScreen(): JSX.Element {
  ...
  if (viewer === undefined || picks === undefined) return <View className="flex-1 bg-[#fff8f1] items-center justify-center"><ActivityIndicator /></View>;
    ...
    <ScrollView className="flex-1 bg-[#fff8f1]" contentContainerClassName="px-6 pt-16 pb-10 gap-5">
      ...
      <View className="flex-row flex-wrap gap-2">
        {categories.map((item) => {
          ...
          return <Pressable key={item.value} className={`rounded-full px-4 py-2 ${active ? "bg-[#2f211c]" : "bg-white/80 border border-[#e6d2c2]"}`} onPress={() => toggle(item.value)}><Text className={active ? "text-white" : "text-[#6f5a50]"}>{item.label}</Text></Pressable>;
        })}
      </View>
      {picks.map((pick) => (
        <View key={pick._id} className="rounded-3xl bg-white/90 p-5 border border-[#f1dfd2] gap-2">
          <Text className="text-sm font-bold uppercase tracking-widest text-[#8c766b]">{pick.category}</Text>
      ...
      {!picks.length ? <Text className="text-base text-[#6f5a50]">No picks in those categories yet.</Text> : null}
    </ScrollView>
  );
}
