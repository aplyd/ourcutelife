      ...
      <ScrollView className="flex-1" contentContainerClassName="px-6 pt-16 pb-32 gap-6">
        <View className="flex-row items-start justify-between gap-4">
          <View className="flex-1 gap-2">
            ...
            <Text className="text-base leading-6 text-[#6f5a50]">
              Suggestions stay private until both of you swipe yes.
            </Text>
          </View>
          <View className="flex-row gap-2 pt-1">
            <Pressable
              className="h-11 w-11 rounded-full bg-white border border-[#e6d2c2] items-center justify-center"
        ...
        <View className="flex-row flex-wrap gap-3">
          {categories.map((category) => (
            <Pressable
              key={category.value}
              className="w-[48%] aspect-square rounded-[32px] bg-white/90 border border-[#f1dfd2] p-5 justify-between"
              onPress={() => router.push(`/plans/match/${category.value}`)}
            >
              ...
              <View className="gap-1">
                ...
                <Text className="text-sm leading-5 text-[#6f5a50]">
                  Swipe privately. Matches reveal later.
                </Text>
              </View>
            </Pressable>
          ))}
